...
function calculateGrid(boardData, people) {
  ...
  // --- PASS 1: REIHENFOLGE & GRUPPEN ---
  sortedCards.forEach((card, index) => {
    if (processed.has(card.label)) return;

    // FIX 1: Gruppen-Sprecher. Nur das letzte Mitglied in der Liste darf die Gruppe platzieren.
    if (card.gruppe) {
        ...
        if (!isLast) return; // Warten auf das letzte Mitglied
    }
  ...
  placedCards.sort((a,b) => a.row - b.row).forEach(pCard => {
    ...
    // FIX 2: Crash-Schutz. Wenn Felder leer sind, werden sie zu 0.
